...
import feh.tec.agents.light.spec.AgentProps
...
import feh.tec.agents.light.spec.macros.AgentPropsBundleBuilder.{DependencyBuild, Dependency, DependsOnSystemAgent}
...
//    val ExtractRaw(variablesTree, negotiationsTree, agentsTree, spawnsTree, timingsTree, timeoutsTree) = raw
    ...
    val bb = new AgentPropsBundleBuilder[c.type](c)
...
object AgentPropsBundleBuilder{
  trait Dependency
...
class AgentPropsBundleBuilder[C <: whitebox.Context](val c: C){
  import c.universe._

  def dependencies(definition: Raw.AgentDef[c.type]): Seq[AgentPropsBundleBuilder.Dependency] = ???
