...
object AgentSpecificationBuilder {
  def build(c: whitebox.Context)(dsl: c.Expr[spec.dsl.Agent]): c.Expr[spec.AgentSpecification] = {
    ...
    lazy val SpecName = "feh.tec.agents.lite.spec.dsl.Agent"

    val builders = Map(
      SpecName -> syntaxBuilder,
      "feh.tec.agents.lite.spec.dsl.PriorityAndProposalBased" -> new PriorityAndProposalBasedBuilder[c.type](c)
    )
